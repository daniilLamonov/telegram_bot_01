...
class ChatRepo(BaseRepository):
    ...
    @classmethod
    async def update_balance(cls, chat_id: int, balance_rub: float, balance_usdt: float):
        await cls._execute('''
                            UPDATE chats
                            SET balance_rub  = $1,
                                balance_usdt = $2,
    ...
    @classmethod
    async def add_to_balance(cls, chat_id: int, amount_rub: float, amount_usdt: float = 0.0):
        await cls._execute('''
                            UPDATE chats
